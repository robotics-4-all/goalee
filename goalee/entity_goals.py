...
class EntityStateCondition(Goal):
    ...
    def evaluate_condition(self, entities):
        ...
        """
        try:
            # SAFETY: This eval() is intentional — it executes condition expressions
            # generated by the GoalDSL code generator. Do NOT use with untrusted input.
            return bool(eval(self._condition, {"entities": entities}, CONDITION_FUNCTIONS))
...
class EntityAttrStream(Goal):
    ...
    def tick(self):
        ...
        if self._attr not in self._entity.attributes:
            raise ValueError(
                f"Attribute {self._attr} not found in entity {self._entity.name}"
            )

        for i, v in enumerate(self._value):
            if self._value_check_list[i]:
                continue
    ...
    def process_for_ordered_strategy(self):
        idx = next(
            (
                i
                for i in reversed(range(len(self._value_check_list)))
                if self._value_check_list[i]
            ),
            -1,
        )
